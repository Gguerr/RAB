    ...
    def add_signature_table
      ...
      @pdf.move_down 10
        ...
        def add_vacation_request_form
          ...
          # Fechas de vacaciones desde el formulario o datos del empleado
          vacation_start = if @filters['vacation_start_date'].present?
                            Date.parse(@filters['vacation_start_date'])
                          else
                            employee&.vacation_date || Date.new(2025, 9, 15)
                          end
          
          vacation_end = if @filters['vacation_end_date'].present?
                          Date.parse(@filters['vacation_end_date'])
                        else
                          vacation_start + 15.days
                        end
      ...
      @pdf.table([
        [
          ...
          { content: "PERÍODO DE DISFRUTE SOLICITADO", font_style: :bold, align: :center, colspan: 8 }
        ],
        [
          { content: "DIA", font_style: :bold, align: :center },
          { content: "MES", font_style: :bold, align: :center },
          { content: "AÑO", font_style: :bold, align: :center },
          { content: "CORRESPONDIENTE AL AÑO 2025", font_style: :bold, align: :center, colspan: 8 }
        ],
          [
        ...
        [
          ...
          { content: "", align: :center , borders: [:left, :right]},
          { content: "", align: :center , borders: [:left, :right]},
          { content: "DIA", font_style: :bold, align: :center },
          { content: "MES", font_style: :bold, align: :center },
          { content: "AÑO", font_style: :bold, align: :center },
          ...
          [
            { content: (@filters['expired_vacations'].present? ? @filters['expired_vacations'] : (employee&.expired_vacations || 15)).to_s, align: :center, colspan: 3, borders: [:left, :right, :bottom] },
            { content: (@filters['vacation_days'].present? ? @filters['vacation_days'] : (employee&.vacation_days || 15)).to_s, align: :center, borders: [:left, :right, :bottom] },
      ...
      @pdf.table([
        [
          { content: "REINCORPORACIÓN EFECTIVA DEL TRABAJADOR", font_style: :bold, align: :center, colspan: 3, borders: [:left, :right, :top] },
          { content: "DIA", font_style: :bold, align: :center, borders: [:left, :right, :top] },
          { content: "MES", font_style: :bold, align: :center, borders: [:left, :right, :top] },
          { content: "AÑO", font_style: :bold, align: :center, borders: [:left, :right, :top] }
        ],
        [
        ...
        columns(0).style(width: 200)  # REINCORPORACIÓN (colspan 3)
        columns(1).style(width: 200)  # REINCORPORACIÓN (colspan 3)
        columns(2).style(width: 200)  # REINCORPORACIÓN (colspan 3)
        columns(3).style(width: 64)   # DIA
        columns(4).style(width: 64)   # MES
        columns(5).style(width: 74)   # AÑO
      end
      ...
      @pdf.table([
        [
          ...
          { content: (@filters['vacation_notes'].present? ? @filters['vacation_notes'] : (employee&.vacation_notes || "SOLO PARA EFECTOS DE DISFRUTE")), align: :center }
        ]
      ], width: @pdf.bounds.width, cell_style: { 
        size: 9, padding: [4, 6, 4, 6], border_width: 1, border_color: '000000'
      }) do
